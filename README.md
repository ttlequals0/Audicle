...
- `reader`: fetch the article through the [Jina Reader](https://jina.ai/reader) proxy, which returns clean markdown and gets past DataDome/PerimeterX bot walls (e.g. wsj.com) that FlareSolverr can't solve -- those answer a scrape with a 401 challenge, not a real page. The endpoint is `READER_PROXY_TEMPLATE` (must contain `{url}`); the keyless public endpoint is rate limited, so if it starts handing back empty or truncated bodies, get a free key at [jina.ai/reader](https://jina.ai/reader) and set `READER_API_KEY`. Both are tunable live in Settings -> Connections (or `PUT /api/v1/settings`), so no restart -- the key is stored masked.

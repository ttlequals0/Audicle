...
The chunk WAVs are joined with ffmpeg (silence handling, loudness normalization, MP3 encode, all tunable under Audio output in Settings). Artwork is fetched from the article or falls back to the show image, and gets embedded in the MP3 as well as served in the feed. The WebVTT transcript is built from the chunk timeline; [chapters](feeds-and-podcasting.md#chapters) come from the same timeline. Finalize writes the episode row and the feed picks it up.
